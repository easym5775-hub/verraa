...
import { Database, KeyRound, LogOut, RotateCw, User } from "lucide-react";
...
export function SettingsView() {
  const { me, isDemo, state, updateCoachName, resetData, toast } = useApp();
      ...
      <div className="mt-6 grid gap-4 lg:grid-cols-2">
        ...
        <SectionCard title="Connection" icon={<Database className="h-4.5 w-4.5" />} bodyCls="p-5">
          <div className="flex items-center gap-3">
            <span className={`h-2.5 w-2.5 rounded-full ${isDemo ? "bg-warn-400" : "bg-moss-400"} tick-pulse`} />
            <p className="font-display text-xl font-bold uppercase text-mist-100">{isDemo ? "Demo mode" : "Supabase · live"}</p>
          </div>
          <p className="mt-2 text-xs leading-5 text-mist-400">
            {isDemo
              ? "Data lives in this browser only (localStorage). Set VITE_SUPABASE_URL and VITE_SUPABASE_ANON_KEY, run the SQL migrations and deploy the edge function to go live."
        ...
        <SectionCard title="Data" icon={<RotateCw className="h-4.5 w-4.5" />} bodyCls="p-5">
          {isDemo ? (
            <>
              <p className="text-xs leading-5 text-mist-400">
                Restore the original demo roster (3 clients with seeded plans, meals, check-ins and chats). Your current
                demo data will be replaced.
              </p>
              {confirmReset ? (
                <div className="mt-4 flex gap-2">
                  <button className={`${btnDanger} ${btnSm} flex-1`} onClick={() => { setConfirmReset(false); void resetData(); }}>
                    Yes, reset it
                  </button>
                  <button className={`${btnSecondary} ${btnSm} flex-1`} onClick={() => setConfirmReset(false)}>
                    Cancel
                  </button>
                </div>
              ) : (
                <button className={`${btnSecondary} ${btnSm} mt-4`} onClick={() => setConfirmReset(true)}>
                  <RotateCw className="h-3.5 w-3.5" /> Reset demo data
                </button>
              )}
            </>
          ) : (
